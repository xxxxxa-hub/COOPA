...
from src.local_python_executor import BASE_BUILTIN_MODULES, LocalPythonExecutor, PythonExecutor, fix_final_answer_code
...
from src.remote_executors import DockerExecutor, E2BExecutor #TODO: need to make it compatible with terminal_tools as well
...
class MultiStepAgent:
    ...
    Args:
        ...
        additional_prompt_variables: variables that will be loaded into the prompt template
        ...
        needs_final_answer (bool, *optional*): If False, skip the forced final_answer phase. # TODO this is not implemented
    """
    ...
    def __init__(
        ...
        additional_prompt_variables: Optional[Dict] = None,
        max_steps: int = 20,
        verbosity_level: LogLevel = LogLevel.INFO,
        save_to_file: Optional[str] = None, # file name to save the conversation log
        ...
        self.additional_prompt_variables = additional_prompt_variables or {}
...
{str(additional_args)}."""
                ...
                "content": [
                    {
                        ...
                        "text": populate_template(
                            self.prompt_templates["planning"]["initial_facts"], variables={"task": task} | self.additional_prompt_variables
                        ),
                    }
                ],
            },
            ...
            "content": [
                {
                    ...
                    "text": populate_template(
                        ...
                        } | self.additional_prompt_variables,
                    ),
                }
            ],
        }
            ...
            "content": [
                {
                    "type": "text",
                    "text": populate_template(
                        self.prompt_templates["planning"]["update_plan_pre_messages"], variables={"task": task} | self.additional_prompt_variables
                    ),
                }
            ],
        }
    ...
    def __call__(self, task: str, **kwargs):
        ...
        full_task = populate_template(
            ...
            variables=dict(name=self.name, task=task) | self.additional_prompt_variables,
        )
        print("=================")
        print(full_task)
        report = self.run(full_task, **kwargs)
        answer = populate_template(
            self.prompt_templates["managed_agent"]["report"], variables=dict(name=self.name, final_answer=report) | self.additional_prompt_variables
        )
        print("=================")
        print(answer)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        agent_dict = {
            ...
            "additional_prompt_variables": self.additional_prompt_variables,  # ✅ Added
    ...
    def from_folder(cls, folder: Union[str, Path], **kwargs):
        ...
        for managed_agent_name, managed_agent_class in agent_dict["managed_agents"].items():
            agent_cls = getattr(importlib.import_module("src.base_agent"), managed_agent_class)
        ...
        args = dict(
            ...
            additional_prompt_variables=agent_dict.get("additional_prompt_variables", {}),  # ✅ Added
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        prompt_templates = prompt_templates or yaml.safe_load(
            importlib.resources.files("src.base_agent_prompts").joinpath("code_agent.yaml").read_text()
        )
        super().__init__(
            tools=tools,
            model=model,
            prompt_templates=prompt_templates,
            additional_prompt_variables=additional_prompt_variables,
            grammar=grammar,
...
class ToolCallingAgent(MultiStepAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        prompt_templates = prompt_templates or yaml.safe_load(
            importlib.resources.files("src.base_agent_prompts").joinpath("toolcalling_agent.yaml").read_text()
        )
        super().__init__(
            tools=tools,
            model=model,
    ...
    def initialize_system_prompt(self) -> str:
        system_prompt = populate_template(
            self.prompt_templates["system_prompt"],
            variables={"tools": self.tools, "managed_agents": self.managed_agents} | self.additional_prompt_variables,
        )
        return system_prompt

    def step(self, memory_step: ActionStep, terminal_tools: Optional[List[str]] = None) -> Union[None, Any]:
        """
        Perform one step in the ReAct framework: the agent thinks, acts, and observes the result.
        Returns None if the step is not final.
        """
